...
AS $$
BEGIN
  INSERT INTO public.profiles (id, name, wallet_address)
  VALUES (
    NEW.id,
    -- For backward compatibility:
    -- 1. If wallet user, use name from metadata or wallet address prefix
    -- 2. If email user, use email prefix (original behavior)
    CASE
      WHEN NEW.raw_user_meta_data->>'wallet_address' IS NOT NULL THEN
        COALESCE(NEW.raw_user_meta_data->>'name', substring(NEW.raw_user_meta_data->>'wallet_address' from 1 for 8))
      ELSE
